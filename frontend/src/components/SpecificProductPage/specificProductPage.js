...
export default function SpecificProductPage() {
    ...
    function getProduct() {
        ...
        fetch(`/api/product?productid=${productID[productID.length - 1]}`)
            .then((res) => res.json())
            .then((data) => {
                if (data && data.length) {
                    setProduct(data);
    ...
    return (
        <>
            {product && product.length ? (
                <div className="mid-container product-page-mid">
                    <div className="left-product-container">
                        <div className="product-photos-container">
                            {currentImage &&
                            currentImage.includes("nike") &&
                            ((product[0].itemType !== "socks" &&
                                product[0].itemType !== "sunglasses") ||
                                currentImage.includes("nike")) ? (
                                <img
                                    id="product-image"
                                    alt=""
                            ...
                            <div className="all-photos-container">
                                {product[0].images.map((img) => {
                                    return (
                                        <img
                                            alt=""
                                            className="smaller-product-image"
                    ...
                    <div className="product-info-container-s">
                        <h5 className="product-type">
                            {product[0].itemType.substring(0, 1).toUpperCase() +
                                product[0].itemType.substring(1, product[0].itemType.length)}
                        </h5>
                        <h1 className="product-name">{product[0].name}</h1>
                        {product[0].colors ? <span>{product[0].colors}</span> : null}
                        ...
                        <div className="sizes-available-container">
                            {product.map((p, i) => {
                                ...
                                return (
                                    <div className="size-container">
                                        <button className="size-span" item={i} onClick={selectSize}>
                                            M {parseFloat(p.size)}/W {parseFloat(p.size) + 1}
                                        </button>
                                    </div>
                                );
                            })}
                        ...
                        <button className="add-to-bag-button" onClick={addItem}>
                            Add To Bag
                        </button>
                    </div>
                </div>
